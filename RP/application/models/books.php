...
class Books extends CI_Model{
	function get($orderBy, $from=0, $perPage=3){
		if ($orderBy == 'nameAsc')
			$query = $this->db->query("SELECT * FROM books ORDER BY name ASC LIMIT $from, $perPage");
		else if ($orderBy == 'nameDes')
			$query = $this->db->query("SELECT * FROM books ORDER BY name DESC LIMIT $from, $perPage");
		else if ($orderBy == 'priceAsc')
			$query = $this->db->query("SELECT * FROM books ORDER BY price ASC LIMIT $from, $perPage");
		else if ($orderBy == 'priceDes')
			$query = $this->db->query("SELECT * FROM books ORDER BY price DESC LIMIT $from, $perPage");
		else
			$query = $this->db->query("SELECT * FROM books LIMIT $from, $perPage");
		return $query->result();
	}
	...
	{
		$data = array(
			'review' => strip_tags($this->input->post('review_textarea')),
			'book_id' => $id,
			'author' => $author
		);
